...
- **Trigger signal vs operational body.** Path-scoped operational rules move to `.claude/rules/<name>.md` with `globs:` frontmatter — body fires when a matching file opens. Trigger phrases (recognition signals) stay ambient in `CLAUDE.md` — fires when intent is recognized. Different moments, different layers.
