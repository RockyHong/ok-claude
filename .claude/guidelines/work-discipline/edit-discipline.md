...
Applies to: any project using AI assistants with text-replace tools, especially Claude Code's `Edit` with `replace_all`.

Does not prescribe: language-specific LSP setup, exact build/test commands, CI integration — repo-shaped (Tier 2). Banned-terms list is conservative universal default; project may extend with domain-specific short identifiers.
