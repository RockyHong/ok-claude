...
Node 20+ + TypeScript CLI, npx-distributed (npm registry). Output: self-contained HTML wordcloud (pure CSS, vanilla JS, browser-side PNG export via `html-to-image`). CJK + Latin tokenization via native `Intl.Segmenter`.
...
> **Two kinds of specs:** `docs/specs/` = permanent source of truth (not scaffolded for this project — promote if features grow ≥5). `docs/superpowers/specs/` = temporal work orders.
