...
| `src/denoise.ts` | Pre-tokenize text cleanup. `denoiseMarkdown(text)` strips: fenced ``` blocks (terminated + unterminated), 4-space-indented code blocks (CommonMark blank-line rule), non-fenced paste blocks (runs of 3+ consecutive stack-frame / type-error / identifier-dense lines — Java `at`, Python `File ".."`, TS `Type '...' is not assignable`, Unity Mono JIT, Gradle `> Task` output, PascalCase namespaces), inline `` `code` `` spans, and English clitic suffixes (`'s`/`'t`/`'d`/`'m`/`'re`/`'ve`/`'ll`, straight + curly apostrophe) so segmenter doesn't emit clitic-fragment tokens (`re` from `we're`, `s` from `it's`). Run before tokenize (GAP-002 / GAP-009 D2). |
