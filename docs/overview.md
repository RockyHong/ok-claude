...
| `src/pipeline.ts` | Orchestrator: discover → stream → per-event denoise → `firstOpener` fold into per-role `OpenerMap` (drives cloud) AND tokenize-and-fold raw token occurrences into per-role frequency Maps (latent restore-bait per DEBT-006 — output unused by render) → `topNOpeners` (100) reshaped to `Array<[surface, count]>` for `topUser`/`topClaude` → render → write. Accumulates messages, token sums, and min/max timestamp inline. Output path resolved via `outputDir()` (probes `~/Downloads` via `statSync().isDirectory()`, falls back to `process.cwd()` on miss / throw) + `outputFilename(stamp)` (`ok-claude-result-${stamp}.html`). Timestamp built once via `buildStamp()` (local time, zero-padded `YYYY-MM-DD-HHMM`), threaded into BOTH `outputFilename` and `RenderInput.meta.timestamp` so the HTML filename pairs with the F5 PNG filename. Username extracted once via `getUsername()` — priority chain: (1) `git config --get user.name` slugged (lowercase, NFKD strip, alphanum-only via `slugName()`), (2) `os.userInfo().username` slugged unless it matches `GENERIC_OS_USERNAMES` regex (`User`, `Administrator`, `admin`, `Default User`, `root`, `nobody`), (3) `"you"` fallback. Passed as `RenderInput.meta.username` so the artifact's `@handle` reflects the actual user without a flag — git name preferred because the OS account is often a generic 'User' on Windows. Returns `{outPath}` or `{outPath: null, reason}`. |
...
| `src/render.ts` | HTML template — tabloid visual lock (DEBT-005). Inlines vendored `wordcloud2.js` and two `<canvas class="cv">` elements (`#canvas-user` + `#canvas-claude`) inside dual `.halves` grid with absolute `.divider` (2px ink-1 between) on a 1080×1080 `.artifact`. Warm-ink ladder `:root` vars: `--paper #0d0d0a` / `--ink-1 #f4f1ea` / `--ink-2 #8a857c` / `--ink-3 #3a3a35` / `--amber #d97757` / `--rule #f4f1ea`. Two type sub-systems via Google Fonts (`fonts.googleapis.com` — see GAP-015): UI tier Anton (headline) + Archivo Narrow 400/500/700 (sub-line, labels, footer) + JetBrains Mono 700 (CTA); Cloud tier Inter 800 only (workhorse). Single-line `.hdr-top` `white-space: nowrap` + `fitHeadline()` JS shrink-loop (start 88px, floor 24px, guard 120 iter) auto-fits headline width. `<span class="ok">ok.</span> CLAUDE: <num>X tokens</num> burned in <num>Y days</num>.` — speaker-label wordmark: `.ok` span overrides parent `text-transform: uppercase` to lowercase + ink-2 dim (whisper-face inside the same Anton family — uses Anton's lowercase glyphs at headline size; no second font); `CLAUDE:` stays caps via parent transform on ink-1 (shout-label); amber-num accents on stats. Reads as session-log header (user mutters → system speaks) mirroring the dual-half user-vs-claude artifact below. `.hdr-bot` `<handle>@${username}</handle> · avg <num>Z tokens</num>/day.` Archivo Narrow 22px ink-2 scaffold (centered, matches CTA centerline); `.handle` is JetBrains Mono 700 ink-1 (matches CTA face) for the OS username extracted in pipeline; ink-1 underlined `<num>` accents on the rate. Double-rule `.hdr-rule` (4px primary + 1px secondary via `::after`). `.labels` grid both left-aligned over each half (reading order, user side amber `<span class="n">` message-count accent / claude side ink-1; claude half pads 14px off the central divider so the label sits inside its column). `.halves` uses grid `grid-template-columns: 1fr 1fr` with an intentional asymmetric gutter: user side flush to the divider (no padding — dense word mass crowds the spine for a chaotic-stream feel), claude side `padding-left: 14px` (organized response sits with breathing room; 14px matches the recurring vertical-rhythm unit already used by `.hdr-bot` / `.halves` / `.footer` margin-tops). Divider absolute-centered via `transform: translateX(-50%)`. Asymmetry is editorial — mirrors the user-noisy / claude-tidy dynamic the artifact roasts. Per-side `drawHalf()` cloud opts: `gridSize: 6`, `weightFactor: logScale(entries, fontMin*dpr, fontMax*dpr)`, `rotationSteps: 0`, `minRotation: -π/9, maxRotation: π/9`, `shuffle: false`, `shrinkToFit: true`, `drawOutOfBound: false`, `caseFn` uppercase, origin inner-edge `24*dpr`. User: fontMin 16 / fontMax 240 / `rotateRatio: 0.35` / color `#f4f1ea`. Claude: fontMin 16 / fontMax 200 / `rotateRatio: 0` / color `#d97757`. Footer is a 3-col grid (`1fr auto 1fr`) under a 1px ink-1 rule: empty-left, centered monospace CTA `▸ npx ok-claude # confess yours` (JetBrains Mono 700, amber chevron, ink-1 command + ink-2 trailing `.cmt` shell comment), and a right-aligned `.byline` creator credit `by rocky hong` styled to match the CTA's `.cmt` shell-comment span (JetBrains Mono 16px ink-2 weight 400, nowrap) so it reads as a footer-line peer, not as background noise. `whenFontsReady()` gate (`document.fonts.ready`) before first render; double-`requestAnimationFrame` first paint post-`load`. JSON-encodes `{topUser, topClaude, meta}` into `window.__DATA__`. F5 wraps the artifact in a `.page` flex container with sibling `.stage` (holds `#artifact`, the PNG capture target) + `.chrome` (centered `.actions` row with `▸ DOWNLOAD` / `▸ COPY` buttons in palette-locked JetBrains Mono 700, amber chevron primary on download, ink-1 secondary; `.toast` slot below in Archivo Narrow 14px ink-2, fade 200ms, hold 2500ms, single re-armable timer). No-scroll body (`overflow: hidden`); `.artifact` keeps native 1080×1080 design grid but scales for display via CSS `transform: scale(var(--s))` with `transform-origin: top left`; `.stage` reserves the scaled box via `calc(1080px * var(--s))`. `fitStage()` computes `--s` on `load` + debounced `resize` from viewport less padding / gap / chrome-axis size. Responsive media query `@media (max-aspect-ratio: 1/1)` flips `.page` to column + `.actions` to row (portrait → buttons under artifact; landscape → buttons right of artifact). `downloadPng()` calls `window.htmlToImage.toBlob` with `pixelRatio:2 backgroundColor:#0d0d0a cacheBust:true style:{transform:'none'}` (style override unscales the clone so the export stays native 2160×2160 regardless of on-screen scale); triggers synthetic `<a download>` with name `ok-claude-result-${meta.timestamp}.png` paired to the HTML filename. `copyPng()` writes a `ClipboardItem({'image/png': blob})` via `navigator.clipboard.write`; on `ClipboardItem`-undefined / throw, toast falls back to `copy not supported — try download instead` (sentinel-rejected promise chain skips success-toast after capture failure). `RenderInput.meta.timestamp` field threaded from pipeline. Inlines vendored `html-to-image.js` alongside `wordcloud2.js`. |
